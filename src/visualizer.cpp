...
void dynamic_callback(ycb_multicam_dataset::visualizerConfig &config) {
    // TODO: Check scene
    config_lock.lock();
    if (scene_ != config.scene || position_ != config.position)
    {
    ...
    for(rosbag::MessageInstance const m: rosbag::View(bag))
    {
        if (m.getTopic() == "cloud")
        {
            ...
            if (cloud != NULL)
            {
                ...
                cloud_pub_.publish(cloud_all_rig_frame);
            }
        }
        if (m.getTopic() == "rgb_img")
        {
            ...
            if (rgb_image != NULL)
            {
                rgb_img_pub_.publish(rgb_image);
            }
        }
        ...
        if (m.getTopic() == "depth_img")
        {
            ...
            if (depth_image != NULL)
            {
                depth_img_pub_.publish(depth_image);
            }
        }
